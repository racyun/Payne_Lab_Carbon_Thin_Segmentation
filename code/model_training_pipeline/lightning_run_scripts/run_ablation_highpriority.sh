#!/usr/bin/env bash
# High-priority augmentation ablation sweep, run sequentially with no stoppage.
#   augmentations: rare_crop (5 levels), affine (4 levels), color_jitter (4 presets) + baseline
...
#
# Usage (inside a tmux session):   bash code/model_training_pipeline/lightning_run_scripts/run_ablation_highpriority.sh
set -u
...
MASTER_LOG="$OUT_ROOT/_sweep_progress.log"

mkdir -p "$OUT_ROOT"
cd "$REPO_ROOT" || { echo "repo not found: $REPO_ROOT"; exit 1; }

# --- preflight checks (fail fast rather than waste a whole sweep) ---
...
# "aug_type level"   (level '-' = not applicable)
EXPERIMENTS=(
  "none -"
  "rare_crop 0.0"
  "rare_crop 0.25"
  ...
  "color_jitter medium"
  "color_jitter strong"
  "color_jitter very_strong"
)

echo "=== ablation sweep started $(date) | ${#EXPERIMENTS[@]} experiments ===" | tee -a "$MASTER_LOG"
i=0
for spec in "${EXPERIMENTS[@]}"; do
  i=$((i + 1))
  set -- $spec
  AUG=$1; LEVEL=$2
  if [ "$AUG" = "none" ]; then NAME="abl_baseline"; else NAME="abl_${AUG}_${LEVEL}"; fi
  LEVEL_ARG=""; [ "$LEVEL" != "-" ] && LEVEL_ARG="--aug_level $LEVEL"
...
echo "=== ablation sweep COMPLETE $(date) ===" | tee -a "$MASTER_LOG"
